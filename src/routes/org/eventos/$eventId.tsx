...
import { useOrgToggleEventStatus, useMyOrganization } from '@/hooks/use-organizer'
...
export const Route = createFileRoute('/org/eventos/$eventId')({
...
function TicketTypeCard({ tt, eventId }: { tt: any; eventId: string }) {
  ...
  return (
    <Card>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              ...
              {activeTier && (
                <span className="ml-2 text-green-600 font-medium">· {formatARS(activeTier.price_ars)} vigente</span>
              )}
            </p>
          </div>
      ...
      {tiers.length === 0 && (
        <CardContent className="pt-0">
          <p className="text-xs text-muted-foreground italic px-2">Sin precios configurados — los usuarios verán "—" al inscribirse.</p>
        </CardContent>
      )}
    </Card>
  )
...
function OrgEventDetailPage() {
  ...
  const { mutate: toggleStatus, isPending: toggling } = useOrgToggleEventStatus(org?.id ?? '')
