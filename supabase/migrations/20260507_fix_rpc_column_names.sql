-- ============================================================
-- Fix create_registration_with_order after multitenant migration
-- ticket_types (ex event_distances), buyer_id (ex runner_id),
-- ticket_type_id (ex distance_id)
-- ============================================================

CREATE OR REPLACE FUNCTION create_registration_with_order(
  p_event_id UUID,
  p_distance_id UUID,   -- kept as p_distance_id for frontend compatibility
  p_user_id UUID,
  p_first_name TEXT,
  p_last_name TEXT,
  p_phone TEXT,
...
DECLARE
  ...
  v_bib INT;
BEGIN
  -- Check capacity with row lock (ticket_types, ex event_distances)
  PERFORM 1
  FROM ticket_types
  ...
  -- Check not already registered (buyer_id, ex runner_id)
  IF EXISTS (
  ...
  -- Get active pricing tier (ticket_type_id, ex distance_id)
  ...
  -- Create registration (ticket_type_id + buyer_id)
  ...
  -- Increment registered_count in ticket_types
